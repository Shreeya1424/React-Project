...
function Faculty() {
    ...
    useEffect(() => {
        ...
        fetch(apiUrl)
            .then(res => {
                if (!res.ok) {
                    throw new Error(`Failed to fetch faculty: ${res.statusText}`);
                }
                return res.json();
            })
            .then(res => {
    ...
    const formattedFaculty = data.map((fac) => (
        <tr key={fac.FacultyID}>  {/* Ensure a unique key */}
            ...
            <td>
                <Link className="btn btn-info" to={`/faculty/${fac.FacultyID}`}>Read More</Link>
            </td>
        </tr>
    ));
    ...
    return (
        <>
            ...
            <table className="table">
                ...
                <tbody>
                    {data.length > 0 ? (
                        formattedFaculty // Render faculty if available
                    ) : (
                        <tr>
